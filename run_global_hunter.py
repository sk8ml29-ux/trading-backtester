...
Examples:
    python run_global_hunter.py --once                 # one scan+filter pass, print decisions, no execution
...
def build_hunter(capital_sek: float) -> GlobalValueHunter:
    ...
    execution_engine = DynamicExecutionEngine(
        adapters={"paper": PaperExecutionAdapter()}, default_adapter="paper", ledger=ledger,
    )
    return GlobalValueHunter(scanner, legal_filter, execution_engine, ledger)
        ...
        if args.once:
            ...
            for d in decisions:
                if isinstance(d, ApprovedOrder):
                    print(
                        f"  APPROVED {d.opportunity.source:<28} {d.opportunity.instrument:<12} "
                        f"size={d.size_sek:>9,.0f} SEK  net_profit={d.expected_net_profit_sek:>8,.2f} SEK"
                    )
                else:
                    print(f"  REJECTED {d.opportunity.source:<28} {d.opportunity.instrument:<12} reason={d.reason}")
            return 0
        else:
...
def main() -> int:
    ...
    mode.add_argument("--once", action="store_true", help="Single scan+filter pass, no execution loop")
    mode.add_argument("--serve", action="store_true", help="Run scanner+filter+execution forever (paper adapter)")
