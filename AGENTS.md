...
- **Global Value Hunter & Arbitrage Bot** — `python run_global_hunter.py --once --capital 400000` (single scan+filter pass, prints decisions, no orders placed) or `--serve` to run the scanner/filter/execution loop forever. Separate package at `global_hunter/` (see `.cursorrules` for the architecture mandate); paper-only by default, writes to `data/live/global_hunter/`. Run `python -m global_hunter.model` to print the aggregated 30 000 SEK/month math model.
...
- Dependency versions float (`pandas>=2.0`, `numpy>=1.24`); the code runs on current pandas 3.x / numpy 2.x. `aiohttp>=3.9` was added for `global_hunter/`'s async venue adapters (everything else in the repo uses stdlib `urllib` via `backtest/http_client.py`).
